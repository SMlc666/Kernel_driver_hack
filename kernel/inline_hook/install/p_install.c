...
int inline_hook_init(void){
	...
	if(P_SYM(p_kallsyms_lookup_name)){
		...
		p_print_log("Warning: kallsyms not available, x86 specific functions must be provided directly\n");
	}
#endif
    ...
    if(P_SYM(p_kallsyms_lookup_name)){
        ...
        p_print_log("Warning: kallsyms not available, ARM specific functions must be provided directly\n");
    }
#endif
	...
	if(P_SYM(p_kallsyms_lookup_name)){
		...
		p_print_log("Warning: Without kallsyms, module_alloc and set_memory_x must be provided directly\n");
	}
...
int p_install_hook(struct p_hook_struct * p_current_hook_struct,char* p_current_hook_state,int p_is_sys){
	...
	if((*p_current_hook_state)==true) return 0;

	/* Priority: target_addr > name lookup */
	if(p_current_hook_struct->target_addr){
		/* Direct function pointer mode */
		p_current_hook_struct->addr = (char*)p_current_hook_struct->target_addr;
		p_print_log("Using direct function pointer: %lx\n", (unsigned long)p_current_hook_struct->addr);
	}else if(p_current_hook_struct->name && strlen(p_current_hook_struct->name) > 0){
		/* Name lookup mode (requires kallsyms) */
		if(!P_SYM(p_kallsyms_lookup_name)){
			p_print_log("kallsyms_lookup_name not available, cannot lookup %s\n", p_current_hook_struct->name);
			return -1;
		}
		p_current_hook_struct->addr=(char*)P_SYM(p_kallsyms_lookup_name)(p_current_hook_struct->name);
		if(!p_current_hook_struct->addr){
			if(p_is_sys && P_SYM(p_sys_call_table)){
				p_current_hook_struct->addr=(void *)P_SYM(p_sys_call_table)[p_current_hook_struct->sys_call_number];
			}else{
		...
		p_print_log("No target address or name provided for hook\n");
		return -1;
	}
#if defined(CONFIG_ARM64) || defined(CONFIG_ARM)
    if(!check_function_length_enough(p_current_hook_struct->addr) || !check_target_can_hook(p_current_hook_struct->addr)){
        p_print_log("[%s/%lx] can not hook\n",p_current_hook_struct->name ? p_current_hook_struct->name : "direct", (unsigned long)p_current_hook_struct->addr);
        return -1;
    }
#endif
	stub=P_SYM(p_module_alloc)(sizeof(hook_stub));
	if(!stub){
		p_print_log("%s module_alloc failed\n",p_current_hook_struct->name ? p_current_hook_struct->name : "direct");
		return -1;
	}
	...
	if(!p_ret){
		...
		p_print_log("%s addr:%lx hook addr:%lx\n",p_current_hook_struct->name ? p_current_hook_struct->name : "direct",(unsigned long)p_current_hook_struct->addr,(unsigned long)stub);
