...
static inline uint64_t read_wb_reg(int reg, int n)
{
	uint64_t val = 0;
...
static inline void write_wb_reg(int reg, int n, uint64_t val)
{
	switch (reg + n) {
...
static inline uint64_t calc_hw_addr(const struct perf_event_attr* attr, bool is_32bit_task) {
...
static inline bool toggle_bp_registers_directly(const struct perf_event_attr * attr, bool is_32bit_task, int enable) {
