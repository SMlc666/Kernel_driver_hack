...
// Define function pointer type and declare the pointer as external.
// It will be resolved at runtime using kallsyms.
typedef u64 (*read_sanitised_ftr_reg_t)(u32 id);
extern read_sanitised_ftr_reg_t read_sanitised_ftr_reg_ptr;

// Replace calls to the unexported function with our function pointer.
// This must be done before including the header that uses the function.
#define read_sanitised_ftr_reg(id) read_sanitised_ftr_reg_ptr(id)
