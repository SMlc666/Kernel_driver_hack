...
int main() {
    ...
    // 认证
    if (ioctl(fd, c_driver::OP_AUTHENTICATE, 0) < 0) {
    ...
    // 暂停子进程
    c_driver::THREAD_CTL tc;
    tc.tid = child_pid;
    tc.action = c_driver::THREAD_ACTION_SUSPEND;
    
    if (ioctl(fd, c_driver::OP_THREAD_CTL, &tc) < 0) {
    ...
    // 恢复线程
    tc.action = c_driver::THREAD_ACTION_RESUME;
    if (ioctl(fd, c_driver::OP_THREAD_CTL, &tc) < 0) {
